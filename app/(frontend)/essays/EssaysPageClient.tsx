...
function readLinkLabel(linkType: Essay['linkType']): string {
  if (linkType === 'pdf') return 'View PDF'
  if (linkType === 'medium') return 'Read on Medium'
  return 'Read more'
}

// ─── Component ────────────────────────────────────────────────────────────────

export default function EssaysPageClient({ initialEssays }: { initialEssays: Essay[] }) {
    ...
    fetch('/api/medium-feed')
      ...
      .then(data => {
        if (data?.feed) {
          ...
          setEssays([...mediumEssays, ...initialEssays])
        }
      })
